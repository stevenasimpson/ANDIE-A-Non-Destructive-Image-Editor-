...
public abstract class ImageAction extends AbstractAction {
     ...
     * Note that the target is not specified per-action, but via the static member target
     * via setTarget.
     * </p>
     * 
     ...
     */
    ImageAction(String name, ImageIcon icon, String desc, Integer mnemonic) {  //note this is string
        super(name, icon);
        if (desc != null) {
     ...
     * @param mnemonic A mnemonic key to use as a shortcut  (ignored if null).
     */    
    public ImageAction(String name, String icon, String desc, KeyStroke mnemonic) { //not the string also needed to add for export action
